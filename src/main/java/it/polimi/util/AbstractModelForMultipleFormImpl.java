...
public class AbstractModelForMultipleFormImpl implements AbstractModel {
	...
	private String inputPathSelector;
	 ...
	 @Value("${input.reading.access.path.multipleform}")
	 private String inputReadingAccessPathSelector;
	 ...
	 * 
	 * @return a new Collection with block set. Name of the block retrieved from
	 * data model
	 * 
	...
	public Collection createBlock() {
		...
		it.polimi.mapper.viewcomponent.selector.Descriptor viewComponentSelector = (it.polimi.mapper.viewcomponent.selector.Descriptor) viewComponentSelectorService
				.loadViewComponentsFromFile(inputPathSelector).get(0);
		...
		// then navigate data model to set key names,types and sort ordering
		...
		collection.getBlock().getKey().getSortKeys().stream().forEach(
				sk -> sk.setOrdering(transformationMultipleFormImpl.setOrderingSortkey(sk.getReferenceAttributeEntity(),
						entity, viewComponentSelector.getQuery().getHQL())));
		...
		collection.getBlock().getKey().getSortKeys().stream()
				.forEach(sk -> sk.setType(dataModelTransformation.retrieveSortKeyType(sk, entityId, dataModel)));
		return collection;
	}
	 ...
	 * 
	 * @return a new Collection with entries set. Name of the entries retrieved from
	 * data model
	 */
	/*
	 * (non-Javadoc)
	...
	@Override
		public Collection evaluateReadingAccessPaths() {
			//get selector. Information bring here, needed to build block
			//@TODO: now can handle only one selector at a time
			it.polimi.mapper.viewcomponent.selector.Descriptor viewComponentSelector = (it.polimi.mapper.viewcomponent.selector.Descriptor) viewComponentSelectorService.loadViewComponentsFromFile(inputReadingAccessPathSelector).get(0);
			
			collection.getBlock().setEntries(
					transformationMultipleFormImpl.updateEntriesOfCollection(
							transformationMultipleFormImpl.findEntriesToAggregate(viewComponentSelector,dataModel), collection.getBlock().getEntries()));
